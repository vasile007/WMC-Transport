...
import SideNavClient from '../components/SideNavClient.jsx';
import { connectSocket } from '../services/socket.js';
import { getUnreadCount, incrementUnread, subscribeUnread } from '../services/chatUnread.js';
...
export default function ClientLayout() {
  ...
  useEffect(() => {
    ...
    s.emit('help:join', null, () => {});
    const onMessage = (msg) => {
      if (!msg?.from || msg.from === 'client') return;
      if (pathRef.current.startsWith('/client/support')) return;
      const next = incrementUnread();
      setChatUnread(next);
    };
    s.on('help:message', onMessage);
    return () => {
      s.off('help:message', onMessage);
      s.disconnect();
    };
  }, [user?.token]);
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-[#0B0D10] via-[#11151B] to-[#141820] text-[#F5F3EE] overflow-hidden">
      ...
      <header className="h-16 bg-[#0B0D10]/85 backdrop-blur border-b border-white/10 shadow-[0_14px_32px_rgba(0,0,0,0.45)] flex items-center justify-between px-6 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-[#B9B3A8]">Client</span>
        </div>
        <div className="flex items-center gap-4">
          <Link to="/client/support" className="relative inline-flex items-center gap-2 text-sm text-[#E3DED5] hover:text-[#F5F3EE]">
            <MessageCircle size={16} />
            Support
            {chatUnread > 0 && (
              <span className="absolute -top-1 -right-2 h-2.5 w-2.5 rounded-full bg-[#D4AF37] ring-2 ring-[#D4AF37]/40 animate-pulse" />
            )}
          </Link>
          <span className="text-sm text-[#E3DED5]">Hello, <b>{user?.name || 'Client'}</b></span>
